...
try:
    ...
    logging.warning("python-dotenv not installed; .env file support disabled. Install with 'pip install python-dotenv'")
...
async def run(
    ...
    **kwargs,
    ...
    # Load additional environment variables from specified .env file
    env_vars = {}
    ...
    if server_type == "sse":
        ...
        main_app.state.env = {**env_vars, **os.environ.copy()}
        ...
        for server_name, server_cfg in mcp_servers.items():
            ...
            if server_cfg.get("command"):
                ...
                # Merge env variables with priority: .env file, os.environ, config file
                env_from_config = server_cfg.get("env", {})
                
                # First create a base environment dictionary
                merged_env = {**env_vars, **os.environ}
                
                # Process and substitute environment variables from config
                processed_env_config = {}
                for key, value in env_from_config.items():
                    if isinstance(value, str) and value.startswith("${") and value.endswith("}"):
                        env_var_name = value[2:-1]
                        if env_var_name in merged_env:
                            processed_env_config[key] = merged_env[env_var_name]
                        else:
                            logging.warning(f"Environment variable {env_var_name} not found")
                            processed_env_config[key] = value
                    else:
                        processed_env_config[key] = value
                
                # Set the final environment with the processed config values having highest priority
                sub_app.state.env = {**merged_env, **processed_env_config}
                
                # Remove the now redundant code below since we've already handled variable substitution
